...
load("FINAL_3N_Neural_Network_Model.rda")
...
foldcrossvalidation = function(neurons_selected,min_thr_selected, dataset) {
  ...
  for (i in 1:10) {
    nn <- neuralnet(ARRIVAL_DELAY ~ SCHEDULED_DEPARTURE + PRECIPITATION + AIRLINE_DL + AIRLINE_WN + AIRLINE_NK,
                    ...
                    linear.output = FALSE, threshold = min_thr_selected)
...
# Uncomment the following row in order to execute the 10-fold cross validation on the current dataset
# !!! It's commented cause of it requires a big amount of time to calculate it !!! #
#Acc_list = foldcrossvalidation(neurons_selected,min_thr_selected, dataset)
